...
namespace PrescribingSystem.Controllers
{
    public class CustomerReportsController : Controller
    {
        ...
        public CustomerReportsController(ApplicationDbContext context, CustomerReportService reportService, UserManager<ApplicationUser> userManager)
        {
            _context = context;
        ...
        [HttpPost]
        public async Task<IActionResult> CustomerReport(DateTime fromDate, DateTime toDate, string groupBy)
        {
            var userId = _userManager.GetUserId(User);

            var data = await _reportService.GetCustomerReportAsync(userId, fromDate, toDate, groupBy);
            var pdf = _reportService.GenerateCustomerReportPdf(data);

            return File(pdf, "application/pdf", $"CustomerReport_{DateTime.Now:yyyyMMdd}.pdf");
        }
